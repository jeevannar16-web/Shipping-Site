  ...
  return (
    <section className="relative flex min-h-screen flex-col justify-end overflow-hidden pt-28">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-0 h-[60vh] w-[70vw] -translate-x-1/2 bg-teal/5 blur-[140px]" />
        <div className="absolute bottom-0 left-0 h-[40vh] w-[40vw] bg-gold/6 blur-[140px]" />
      </div>
      ...
      {activeCountry && (
        <div className="pointer-events-none absolute right-[6vw] top-[24vh] z-20 hidden max-w-xs animate-pulse rounded-xl border border-teal/30 bg-void/80 p-5 backdrop-blur-xl lg:block">
          <p className="font-display text-sm font-semibold text-teal">{activeCountry.name}</p>
      ...
      <div ref={hubRef} className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-24">
        <p className="mb-6 flex items-center gap-3 text-xs uppercase tracking-[0.3em] text-white/45" data-anim="fade-up">
          <Globe2 size={15} className="text-gold" />
        ...
        <h1 className="max-w-5xl font-display text-5xl font-bold leading-[1.02] tracking-tight text-white sm:text-6xl lg:text-[5.5rem]" data-anim="lines">
          Every leg
          <br />
          <span className="text-glow-gold text-gold">of the journey.</span>
        </h1>
        ...
        <div className="mt-10 flex flex-wrap items-center gap-4" data-anim="fade-up">
          <MagneticButton
            href="#contact"
            className="group flex items-center gap-3 rounded-full bg-gold px-7 py-4 text-sm font-semibold text-void transition-all hover:shadow-[0_0_40px_rgba(245,165,36,0.4)] active:scale-[0.97]"
          >
            Talk With Us
          ...
          <MagneticButton
            href="#services"
            className="flex items-center gap-3 rounded-full border border-white/15 px-7 py-4 text-sm font-semibold text-white transition-all hover:border-teal hover:text-teal active:scale-[0.97]"
          >
            Our Services
          </MagneticButton>
        </div>
        ...
        <div className="mt-16 flex max-w-2xl flex-wrap items-center justify-between gap-6 border-t border-white/10 pt-8" data-anim="fade-up">
          {[
            { value: 'Nepal', label: 'Operational HQ' },
