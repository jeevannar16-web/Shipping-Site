    ...
    <section className="relative flex min-h-screen flex-col justify-end overflow-hidden pt-28">
      ...
      {activeHub && (
        <div className="pointer-events-none absolute right-[6vw] top-[24vh] z-20 hidden max-w-xs animate-pulse rounded-xl border border-cyber-blue/30 bg-void/80 p-5 backdrop-blur-xl lg:block">
          <p className="font-display text-sm font-semibold text-cyber-blue">{activeHub.name}</p>
          <p className="mt-1 text-xs text-white/50">{activeHub.region} Strategic Hub</p>
          <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-white/35">Throughput</p>
              <p className="mt-0.5 font-medium text-white">{activeHub.throughput}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-white/35">Avg. Transit</p>
              <p className="mt-0.5 font-medium text-white">{activeHub.transit}</p>
            </div>
            <div className="col-span-2">
              <p className="text-[10px] uppercase tracking-wider text-white/35">Integration</p>
              <p className="mt-0.5 font-medium text-cyber-blue">{activeHub.integration}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div ref={hubRef} className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-24">
        <p className="mb-6 flex items-center gap-3 text-xs uppercase tracking-[0.3em] text-white/45">
          <Globe2 size={15} className="text-neon-orange" />
          Global Freight Forwarding & Logistics
          <span className="h-px w-12 bg-white/20" />
        </p>

        <h1 className="max-w-5xl font-display text-5xl font-bold leading-[1.02] tracking-tight text-white sm:text-6xl lg:text-[5.5rem]">
          Moving the world,
          <br />
          <span className="text-glow-orange text-neon-orange">ship by ship.</span>
        </h1>

        <p className="mt-8 max-w-xl text-base leading-relaxed text-white/55 md:text-lg">
          United Carriers orchestrates air, ocean, customs and warehousing across 120+ countries —
          with real-time visibility, unbroken cold chain, and a 99.4% on-time network.
        </p>
        ...
        <div className="mt-16 flex max-w-2xl flex-wrap items-center justify-between gap-6 border-t border-white/10 pt-8">
          {[
            { value: '99.4%', label: 'On-Time Network' },
            { value: '120+', label: 'Countries Served' },
            { value: '2.4M', label: 'Shipments / Year' },
            { value: '24/7', label: 'Freight Monitoring' },
          ].map((stat) => (
            <div key={stat.label}>
